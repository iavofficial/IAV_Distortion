...
# Tests
@pytest.mark.asyncio
async def test_track_transistion():
    """
    Test the transition from one track piece onto the next one
    """
    location_service = LocationService(get_two_straight_pieces(), simulation_ticks_per_second=1,
                                       start_immediately=False)
    location_service._set_speed_mm(1, acceleration=1)
    for i in range(0, STRAIGHT_PIECE_LENGTH()):
        await location_service._run_simulation_step_threadsafe()
...
async def test_acceleration(speed, acceleration):
    """
...
async def test_multiple_transitions(speed: float, offset: float):
    """
    Test the transitions against jumping at a full track
    """
    track = get_loop_track()
    location_service = LocationService(track, simulation_ticks_per_second=1, start_immediately=False)
    ...
    old_pos, _ = await location_service._run_simulation_step_threadsafe()
    for _ in range(0, track.get_len() * STRAIGHT_PIECE_LENGTH()):
        new_pos, _ = await location_service._run_simulation_step_threadsafe()
...
async def test_offset(offset: float):
    """
...
@pytest.mark.asyncio
async def test_position_rotation():
    """
    ...
    while True:
        _, rot = await location_service._run_simulation_step_threadsafe()
        if location_service._current_piece_index != 0:
            break
        assert rot.get_deg() == pytest.approx(90)
    _, last = await location_service._run_simulation_step_threadsafe()
    while True:
        _, new = await location_service._run_simulation_step_threadsafe()
